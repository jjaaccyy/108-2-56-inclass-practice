...
table$`國文`
new_levels <-
  levels(table$`國文`)[c(1,2,11,3,4,5,6,7,8,9,10,12,13,14,15,16)]
table$`國文` <- factor(table$`國文`,levels=new_levels,ordered = T)
table$`國文` -> chinese
as.integer(chinese)
...
logical儲存的本質是甚麼?
```{r ans23}
pick_ntpueco <- students10$`志願` == "北大經濟"
pick_1 <- students10$`國文`[pick_ntpueco] >= 12
pick_1
...
paste("恭喜",congrats_names,"同學錄取國立臺北大學經濟系",sep=" ") -> congrats
class(congrats)
