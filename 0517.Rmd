...
```
```{r}
factor(garbageRoutes$linename) -> linenamefactor
factor(stringr::str_extract(garbageRoutes$linename,"[:graph:]+路線")) -> garbageRoutes$route
levels(garbageRoutes$route)

```

```{r}
table(garbageRoutes$route) -> tableroute
sort(tableroute) -> sortroute
names(sortroute)-> namesroute
garbageRoutes$route <- factor(garbageRoutes$route,namesroute)
levels(garbageRoutes$route)
```
```{r}
