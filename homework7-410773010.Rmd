...
```{r ans11}
six_face <- function(x,y,z)
  {2*(x*y+x*z+y*z) -> surface
  return(surface)}
six_face(20,10,15) -> surface_A
...
單號請用1表示、雙號則用0表示。
```{r ans21}
Mask$`身分證字號`[[1]] -> numbers
stringr::str_sub(numbers,10) -> pick_number
as.integer(pick_number) -> pick_number
`1st_odd_or_even` <- pick_number %% 2
...
get_courses <- function(entranceYear, major){
  ...
  str_detect(courseStructure$`應修系級`,paste0(major,"1")) -> pick_class1
   courseStructure$`學年` == as.integer(entranceYear) -> year11   
   pick_class1 & year11 -> pickk1  
   
  str_detect(courseStructure$`應修系級`,paste0(major,"2")) -> pick_class2
   courseStructure$`學年` == as.integer(entranceYear)+1 -> year22 
   pick_class2 & year22 -> pickk2
   str_detect(courseStructure$`應修系級`,paste0(major,"3")) -> pick_class3
 courseStructure$`學年` == as.integer(entranceYear)+2 -> year33 
 pick_class3 & year33 -> pickk3    
 
 str_detect(courseStructure$`應修系級`,paste0(major,"4")) -> pick_class4
  courseStructure$`學年` == as.integer(entranceYear)+3 -> year44    
  pick_class4 & year44 -> pickk4
  
  list(
    year1= unique(courseStructure$`科目名稱`[pickk1])
  ,year2= unique(courseStructure$`科目名稱`[pickk2])
  ,year3= unique(courseStructure$`科目名稱`[pickk3]
  ),year4= unique(courseStructure$`科目名稱`[pickk4]
  )) -> getting
return(getting)}
...
```{r ans34}
as.factor(get_coursesAllYears(100,"經濟系")[[1]][[1]]

list_econCourseCounts <- list(entrane=100:103,
                              year1=c())
